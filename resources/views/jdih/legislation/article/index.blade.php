            ...
            <div class="content">
                <div class="page-header page-header-content d-lg-flex">
                    <div class="page-title">
                        <h2 class="fw-bold mb-0">@isset($category) {{ $category->name }} @else Artikel Hukum @endif</h2>
                    </div>
                    ...
                    <div class="mb-3 my-lg-auto ms-lg-auto">
                        <div class="breadcrumb">
                            ...
                            @else
                                <span class="breadcrumb-item active">Artikel Hukum</span>
                            @endisset
                        </div>
                    </div>
                </div>
                    ...
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            @foreach ($orderOptions as $key => $value)
                                <a
                                @isset ($category)
                                    href="{{ route('legislation.article.category', ['category' => $category->slug, 'order' => $key] + Request::query()) }}"
                                @else
                                    href="{{ route('legislation.article.index', ['order' => $key] + Request::query()) }}"
                                @endisset
            ...
            @forelse ($legislations as $legislation)
                <article class="card card-body shadow mb-4">
                    <div class="d-sm-flex align-items-sm-start">
                        ...
                        <a href="{{ route('legislation.article.show', ['category' => $legislation->category->slug, 'legislation' => $legislation->slug]) }}" class="d-block me-sm-3 mb-3 mb-sm-0">
                        ...
                        <div class="flex-fill">
                            <a href="{{ route('legislation.article.category', $legislation->category->slug) }}" class="badge bg-indigo bg-opacity-10 text-indigo rounded-pill mb-1">{{ $legislation->category->name }}</a>
                            <h4 class="mb-1">
                                <a href="{{ route('legislation.article.show', ['category' => $legislation->category->slug, 'legislation' => $legislation->slug]) }}" class="text-body">{!! Str::highlightPhrase($legislation->shortTitle, Request::get('title')) !!}</a>
                            </h4>
